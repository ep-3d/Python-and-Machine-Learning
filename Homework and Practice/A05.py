"""
# ****************************************************************************
# Author:           Elliott Parker
# Assignment:       Assignment #03
# Date:             August 20, 2023
# Description:      This program takes the user’s first name, location, and a
#                   guess/estimate of the temperature as inputs. An API then
#                   pulls the temperature and humidity for their location, in
#                   as a zip code. I created an API to convert the ZIP to
#                   longitude and latitude coordinates. The program then
#                   calculates the heat index if the temperature is above 80
#                   degrees Fahrenheit. If the temperature is below 50 degrees,
#                   the program will also pull the wind speed. The wind speed
#                   will be used to calculate the wind chill.
#                   (02) Refactoring for functions
#                   (03) Adding while loops, menu and If/Elif/Else statements.
# Revisions:          (3.1) Changed how the menu choice is selected. by
														moving it to the end of while loop. Also
														shortened option 2 so that it goes back to the
														menu immediately after. Removed while true/break.
										(04)	Adding validation steps
											(4.1) Calling validate zip code inside the get_location()
											      function.Return True if the zip code is good
											      False otherwise.
							      (05) parallel lists to hold related information.
							      
# ****************************************************************************
"""
import requests
import valid as v  # double check folder placement

# Constants:
C_1 = 42.379
C_2 = 2.04901523
C_3 = 10.14333127
C_4 = 0.22475541
C_5 = 6.83783e-3
C_6 = 5.481717e-2
C_7 = 1.22874e-3
C_8 = 8.5282e-4
C_9 = 1.99e-6
D_1 = 35.74
D_2 = 0.6215
D_3 = 35.75
D_4 = 0.16
D_5 = 0.4275

# Variables
user_name_list = []
location = []
temperature = []
heat_index = []
wind_chill = []
relative_humidity = []
wind_velocity = []
latitude = [] # Beaverton Oregon = 45.4869   (−90; 90)
longitude = []  # Beaverton Oregon = 122.8040  (-180; 180)
part = "minutely,hourly,daily,alerts"
american_units = "imperial"
zip_code_list = []
country_code = "US"
place = []
fav_list = []
data_valzip = 0


def main():
	welcome()
	choice = menu()
	list_index = 0

	while choice != 5:
		if choice == 1:
			user_name_list.append(get_name())
			zip_code_list.append(get_location())
			data_zip = geocode(zip_code_list[list_index])
			longitude.append(get_longitude(data_zip))
			latitude.append(get_latitude(data_zip))
			place.append(get_place(data_zip))
			data_1 = weather(longitude[list_index], latitude[list_index])
			temperature.append(get_temperature(data_1))
			relative_humidity.append(get_humidity(data_1))
			wind_velocity.append(get_wind_vel(data_1))
			heat_index.append(calc_heat_index(temperature[list_index],
			                             relative_humidity[list_index]))
			wind_chill.append(calc_wind_chill(temperature[list_index],
			                             wind_velocity[list_index]))
			outputs(list_index, temperature, user_name_list, place,
			        wind_chill, heat_index)
			list_index = list_index + 1
			
		elif choice == 2:
			fav_list.append(get_fav_zip_code())
			
		elif choice == 3:
			print_favs = print_fav_list(fav_list)
			print()
			
		elif choice == 4:
			print_table_results(zip_code_list,temperature,
			                    relative_humidity, wind_velocity)
			
		else:
			print("Make Better choices...\n")
		
		choice = menu()  # better way to end my loops
	
	goodbye()


def welcome():
	"""
  Prints a welcome message.
  """
	print("Welcome to the weather App\n")


def get_name():  # Inputs
	"""
	Asks the user to enter their name.
	:return: The name entered by the user. Datatype: String
	"""
	user_name = v.get_string("\nPlease enter your name: ")
	return user_name


def get_location():  # Inputs
	"""
	Asks the user to enter their zip code.
	:return: The zip code entered by the user. Datatype: Integer
	"""
	zip_code = ""
	zip_code = v.get_string("\nPlease enter a zip code: ")
	while len(zip_code) != 5 or zip_code.isnumeric() == False \
			or validate_zip_code(zip_code) == False:
		print("Invalid entry. Please use '#####' format.) ")
		zip_code = v.get_string("\nPlease enter a zip code: ")
		print("Thank you.")
	return zip_code

def geocode(zip_code):  # API for Direct GeoCoding data
	"""
	Retrieves the geographical data (longitude, latitude,
	and place name) for a given zip code using the OpenWeatherMap API.
	:param zip_code: The zip code for retrieval the data.
		Datatype: Integer
	:return: The API response data in JSON format.
	"""
	API_KEY_zip = "9e3f1ef0cc4f44e4089db52f4de0cf0c"
	API_ENDPOINT_zip = "http://api.openweathermap.org/geo/1.0/zip?"
	part_2 = ",".join([str(zip_code), country_code])
	parameters_zip = {"zip": part_2,
	                  "appid": API_KEY_zip}
	response_zip = requests.get(API_ENDPOINT_zip, params=parameters_zip)
	data_zip = response_zip.json()
	return data_zip


def get_longitude(data_zip):  # Storing API Response Results (GeoCoding)
	"""
	Retrieves the longitude from the API response data.
	:param data_zip: The API response data in JSON format.
	:return: The longitude value. Datatype: Float
	"""
	longitude = data_zip["lon"]
	return longitude


def get_latitude(data_zip):  # Storing API Response Results (GeoCoding)
	"""
	Retrieves the latitude from the API response data.
	:param data_zip: The API response data in JSON format.
	:return: The latitude value. Datatype: Float
	"""
	latitude = data_zip["lat"]
	return latitude


def get_place(data_zip):  # Storing API Response Results (GeoCoding)
	"""
  Retrieves the place name from the API response data.
  :param data_zip: The API response data in JSON format.
  :return: The place name. Datatype: String
  """
	place = data_zip["name"]
	return place


def weather(longitude, latitude):  # API For Weather Data
	"""
	Retrieves weather data for a given longitude and latitude
	using the OpenWeatherMap API.
	:param longitude: The longitude for weather data. Datatype: Float
	:param latitude: The latitude for weather data.Datatype: Float
	:return: The API response data in JSON format.
	"""
	API_KEY = "9e3f1ef0cc4f44e4089db52f4de0cf0c"
	API_ENDPOINT = "https://api.openweathermap.org/data/3.0/onecall?"
	Parameters = {"lat": latitude,
	              "lon": longitude,
	              "exclude": part,
	              "units": american_units,  # default units are in kelvin
	              "appid": API_KEY}
	response = requests.get(API_ENDPOINT, params=Parameters)
	data_1 = response.json()  # Reading the API Response (Weather)
	return data_1


def get_temperature(data_1):  # Storing API Response Results (Weather)
	"""
	Retrieves the current temperature from the API response data.
	:param data_1: The API response data in JSON format.
	:return: The temperature value. Datatype: Float
	"""
	temperature = data_1["current"]["temp"]
	return temperature


def get_humidity(data_1):
	"""
	Retrieves the current humidity from the API response data.
	:param data_1: The API response data in JSON format.
	:return: The humidity value. Datatype: Float
	"""
	relative_humidity = data_1["current"]["humidity"]
	return relative_humidity


def get_wind_vel(data_1):
	"""
	Retrieves the current wind velocity from the API response data.
	:param data_1: The API response data in JSON format.
	:return: The wind velocity value. Datatype: Float
	"""
	wind_velocity = data_1["current"]["wind_speed"]
	return wind_velocity


def calc_heat_index(temperature, relative_humidity):  # Calculations
	"""
	Calculates the heat index based on the temperature and relative humidity.
	:param temperature: The current temperature. Datatype: Float
	:param relative_humidity: The current relative humidity. Datatype: Float
	:return: The heat index value. Datatype: Float
	"""
	heat_index = -C_1 + \
	             C_2 * temperature + \
	             C_3 * relative_humidity - \
	             C_4 * temperature * relative_humidity - \
	             C_5 * temperature * temperature - \
	             C_6 * relative_humidity * relative_humidity + \
	             C_7 * temperature * temperature * relative_humidity + \
	             C_8 * temperature * relative_humidity * relative_humidity - \
	             C_9 * temperature * temperature * \
	             relative_humidity * relative_humidity
	return heat_index


def calc_wind_chill(temperature, wind_velocity):  # Calculations
	"""
	Calculates the wind chill based on the temperature and wind velocity.
	:param temperature: The current temperature. Datatype: Float
	:param wind_velocity: The current wind velocity. Datatype: Float
	:return: The wind chill value. Datatype: Float
	"""
	wind_chill = D_1 + \
	             (D_2 * temperature) - \
	             (D_3 * (wind_velocity ** D_4)) + \
	             (D_5 * temperature * (wind_velocity ** D_4))
	return wind_chill


def outputs(list_index, temperature, user_name_list, place,
            wind_chill, heat_index):  # Outputs
	"""
	Displays the weather information and appropriate messages
	based on the temperature.
	:param temperature: The current temperature. Datatype: Float
	:param user_name: The name of the user. Datatype: String
	:param place: The name of the place. Datatype: String
	:param wind_chill: The calculated wind chill value. Datatype: Float
	:param heat_index: The calculated heat index value. Datatype: Float
	"""
	if temperature[list_index] < 51:
		print(f"\nHello {user_name_list[list_index]}. "
		      f"The current temperature in {place[list_index]}"
		      + f" is {temperature:.1f}")
		print(f"\nAnd with the wind chill it feels like "
		      f"{wind_chill[list_index]:.1f}")
	elif temperature[list_index] > 79:
		print(f"\nHello {user_name_list[list_index]}. "
		      f"The current temperature in {place[list_index]}"
		      + f" is {temperature[list_index]:.1f}")
		print(f"\nAnd with the heat index it feels like "
		      f"{heat_index[list_index]:.1f}")
	else:
		print(f"\nHello {user_name_list[list_index]}. "
		      f"The current temperature in {place[list_index]}"
		      + f" is {temperature[list_index]:.1f}")
		print("You should wear a sweater")


def goodbye():
	"""
	Prints a goodbye message.
	"""
	print("Thank you and Goodbye!")


def menu():
	"""
	Displays a menu and prompts the user for a selection.
	:return: The user's selection as an integer. Datatype: Int
	"""
	print("[1] Get Current Weather")
	print("[2] Store Favorite Location")
	print("[3] View Favorites")
	print("[4] New! Print A List!")
	print("[5] Exit\n")
	
	choice = v.get_integer("Please enter your selection: ")
	return choice


def get_fav_zip_code():
	"""
	Prompts the user to enter favorite zip codes and stores them in a list.
	Continues to prompt until the user chooses not to enter a new zip code.
	:return: A list of favorite zip codes. Datatype: List of Int
	"""
	zip_code = ""
	zip_code = v.get_string("\nPlease enter your favorite zip code: ")
	while len(zip_code) != 5 or zip_code.isnumeric() == False:
		print("Invalid entry. Please use '#####' format.) ")
		zip_code = v.get_string("\nPlease enter your favorite zip code: ")
	print("Thank you.")
	return zip_code
	


def print_fav_list(fav_list):
	"""
	Prints the user's favorite zip codes.
	:param fav_list: A list of favorite zip codes. Datatype: List of Int
	"""
	if len(fav_list) == 0:
		print("Empty list. Please enter a new zip code")
	else:
		print("\nHere is a list of your favorite zip codes: \n")
		print(fav_list)


def validate_zip_code(zip_code):
    """
    Validates a given zip code by making a request to an external API and
    extracting relevant information from the response.

    :param zip_code: The zip code to validate. Datatype: int
    :return: True if the zip code is valid, False otherwise. Datatype: bool
    """
    headers = {"apikey": "856c0a70-4202-11ee-a2f1-c3c8f5979336"}
    params = (("codes", f"{zip_code}"), ("country", "us"))

    try:
        response = requests.get('https://app.zipcodebase.com/api/v1/search',
                                headers=headers, params=params)
        data_valzip = response.json()

        if data_valzip.get("results"):
            postal_code = data_valzip["results"][f"{zip_code}"][0]["postal_code"]
            return zip_code == postal_code
        else:
            return False

    except requests.exceptions.RequestException as req_exc:
        print("An error occurred while making the request:", req_exc)
    except KeyError:
        print(f"The provided zip code {zip_code} "
              f"was not found in the response.")
    except Exception as e:
        print("An unexpected error occurred:", e)
        

def print_table_results(zip_code_list, temperature, relative_humidity,
                        wind_velocity):
	"""
    Prints a table of weather results including zip codes, temperature,
    relative humidity, wind speed, wind chill, and heat index.

    :param zip_code_list: A list of zip codes. Datatype: List of Str
    :param T: A list of temperatures. Datatype: List of Float
    :param RH: A list of relative humidity values. Datatype: List of Float
    :param WV: A list of wind velocities. Datatype: List of Float
	"""
	index = 0
	if len(zip_code_list) == 0:
		print("\nNo information available.")
	else:
		print("Zip Code      Temp        Humidity     Wind Speed     WC      HI ")
		print("—-------      —-----      —-------     —---------    ----    ----")
		while index < len(zip_code_list):
			print(f"{zip_code_list[index]: <14}"
			      f"{temperature[index]: <12}"
			      f"{relative_humidity[index]: <13}"
			      f"{wind_velocity[index]: <14}"
			      f"{calc_wind_chill(temperature[index], wind_velocity[index]): <8.2f}"
			      f"{calc_heat_index(temperature[index], relative_humidity[index]): <1.2f}")
			index = index + 1
		
"""
	Problems
	
		(01) Validate_zip_code(), Weather(), and Geocode() are the API functions.
		     I didnt know how best to store the API result in list, when I
		     already had functions to parse out the useful information from the
		     API functions. Or should I have put DataZip_1 into a list?

"""

main()
